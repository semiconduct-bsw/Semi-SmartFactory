// NOT Working : 목록 보기, 검색
// NOT Intend : 수정

using Oracle.ManagedDataAccess.Client;

namespace _20240722_OracleBusinessConnect02
{
    class Businesscard
    {
    ...
    internal class Program
    {
            ...
            do
            {
                ...
                switch (choice)
                {
                    case 1:
                        ...
                        cardList.Add(bus);
                        SaveBusinessCardToDatabase(bus);
                        Console.WriteLine("================================");
                        break;
                    case 2:
                    ...
                    case 3:
                        ...
                        for (int i = 0; i < cardList.Count; i++)
                        {
                            if (cardList[i].ID == select)
                            {
                                ...
                                UpdateBusinessCardInDatabase(cardList[i]);
                            }
                        }
                        break;
                    case 4:
                        ...
                        for (int i = 0; i < cardList.Count; i++)
                        {
                            if (cardList[i].ID == select2)
                            {
                                DeleteBusinessCardFromDatabase(cardList[i].ID);
                                cardList.RemoveAt(i);
                            }
                        }
                        break;
                    case 5:
                        ...
                        for (int i = 0; i < cardList.Count; i++)
                        {
                            ...
                            {
                                Console.WriteLine($"{cardList[i].ID}. {cardList[i].Name} | {cardList[i].HP} | {cardList[i].Email} | {cardList[i].Company} | {cardList[i].Address}");
                            }
                        }
                        break;
                    case 6:
        ...
        static void SaveBusinessCardToDatabase(Businesscard card)
        {
            using (OracleConnection conn = new OracleConnection(strConn))
            {
                conn.Open();
                string sql = "INSERT INTO Businesscards (ID, Name, HP, Email, Company, Address) VALUES (:ID, :Name, :HP, :Email, :Company, :Address)";
                using (OracleCommand cmd = new OracleCommand(sql, conn))
                {
                    cmd.Parameters.Add(new OracleParameter("ID", card.ID));
                    cmd.Parameters.Add(new OracleParameter("Name", card.Name));
                    cmd.Parameters.Add(new OracleParameter("HP", card.HP));
                    cmd.Parameters.Add(new OracleParameter("Email", card.Email));
                    cmd.Parameters.Add(new OracleParameter("Company", card.Company));
                    cmd.Parameters.Add(new OracleParameter("Address", card.Address));
                    cmd.ExecuteNonQuery();
                }
            }
        }

        static void UpdateBusinessCardInDatabase(Businesscard card)
        {
            using (OracleConnection conn = new OracleConnection(strConn))
            {
                conn.Open();
                string sql = "UPDATE Businesscards SET Name = :Name, HP = :HP, Email = :Email, Company = :Company, Address = :Address WHERE ID = :ID";
                using (OracleCommand cmd = new OracleCommand(sql, conn))
                {
                    cmd.Parameters.Add(new OracleParameter("ID", card.ID));
                    cmd.Parameters.Add(new OracleParameter("Name", card.Name));
        ...
        static void DeleteBusinessCardFromDatabase(int id)
        {
            using (OracleConnection conn = new OracleConnection(strConn))
            {
                conn.Open();
                string sql = "DELETE FROM Businesscards WHERE ID = :ID";
                using (OracleCommand cmd = new OracleCommand(sql, conn))
                {
