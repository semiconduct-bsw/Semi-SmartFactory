...
namespace _20241112_CIMPCGUI01
{
    public partial class CIMMainForm : Form
    {
                                ...
                                case "10704": // Canceled Report
                                    ...
                                    if (dataParts.Length >= 5)
                                    {
                                        ...
                                        if (dgvCIM.InvokeRequired)
                                        {
                                            dgvCIM.Invoke(new Action(() =>
                                            {
                                                dgvCIM.Rows.Add(canceledReport.ReportID, canceledReport.ModelID, canceledReport.OPID,
                                ...
                                switch (rcmd)
                                {
                                    case "START":
                                        if (dataParts.Length >= 5)
                                        {
                                            ...
                                            latestData = $"{startRcmd.RCMD}/{startRcmd.ModelID}/" +
        ...
        private void CIMMainForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            PlcReconnectTimer?.Dispose();
            MesReconnectTimer?.Dispose();
            PlcStatusCheckTimer?.Dispose();
            MesStatusCheckTimer?.Dispose();

            LogMessage("연결 종료");
            Environment.Exit(0); // 모든 프로세스를 완전히 종료
        }
