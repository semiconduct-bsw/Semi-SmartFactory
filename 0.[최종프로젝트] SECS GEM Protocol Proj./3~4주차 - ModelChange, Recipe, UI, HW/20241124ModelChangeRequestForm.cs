...
namespace _20241112_CIMPCGUI01
{
    ...
    {
        public CIMMainForm mainForm;

        public ComboBoxForm(CIMMainForm form)
        {
            InitializeComponent();
        ...
        private void btnComboORR_Click(object sender, EventArgs e)
        {
            // mainForm이 null인지 확인
            if (mainForm == null)
            {
                MessageBox.Show("Main Form이 초기화되지 않았습니다.", "오류", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            string modelID = comboModelID.Text.Trim();
            bool isMatched = false;
            foreach (string item in listModelID.Items)
            {
                if (string.Equals(item, modelID, StringComparison.OrdinalIgnoreCase)) // 대소문자 무시 비교
                {
                    isMatched = true;
                    break;
                }
            ...
            mainForm.anoModelID = modelID;

            if (mainForm.serverRealSocket != null && mainForm.serverRealSocket.Connected)
            {
                try
                {
                    string message = $"_10303/{modelID}/{mainForm.anoOPID}/EDS 1";
